...
std::vector<FaceInfo>MTCNN::NonMaximumSuppression(std::vector<FaceInfo>& bboxes,
	...
	for (int i = 0; i < n; i++) {
		areas[i] = (bboxes[i].bbox.xmax - bboxes[i].bbox.xmin + 1)*(bboxes[i].bbox.ymax-bboxes[i].bbox.ymin+1);
		sm.insert(ScoreMapper::value_type(bboxes[i].bbox.score, i));
	}
	vector<FaceInfo> picked;
	while (!sm.empty()) {
		int last_idx = sm.rbegin()->second; //最高分的box
		picked.push_back(bboxes[last_idx]);
		const FaceBox& last = bboxes[last_idx].bbox; //当前的人脸
		for (ScoreMapper::iterator it = sm.begin(); it != sm.end();) { //遍历
			int idx = it->second;
			const FaceBox& curr = bboxes[idx].bbox;
			float x1 = std::max(curr.xmin, last.xmin);
			float y1 = std::max(curr.ymin, last.ymin);
			float x2 = std::min(curr.xmax, last.ymax);
			float y2 = std::min(curr.ymax, last.xmax);
			...
			switch (methodType) {
				...
				case 'm':{
					if (static_cast<float>(w*h) / std::min(areas[idx], areas[last_idx]) > thresh){
						ScoreMapper::iterator it_ = it;
						it_++;
						sm.erase(it);
						it = it_;
...
MTCNN::MTCNN(const string& proto_model_dir) {
	...
	int num_channels_ = PNet_->input_blobs()[0]->channels();
...
vector<FaceInfo> MTCNN::ProposalNet(const cv::Mat& img, int minSize, float threshold, float factor) {
	...
	for (int i = 0; i < scales.size(); i++) {
		...
		std::sort(candidate_boxes_.begin(), candidate_boxes_.end(), CompareBBox);
		total_boxes_.insert(total_boxes_.end(), candidate_boxes_.begin(), candidate_boxes_.size()>=10? (candidate_boxes_.begin() + 10):candidate_boxes_.end());
		/*
...
// 备注:
/*******************************************************/
vector<FaceInfo> MTCNN::Detect(const cv::Mat& image, const int minSize, const float* threshold, const float factor) {
	...
	//cout << "Pnet:" << (float)std::chrono::duration_cast<std::chrono::microseconds>(p1 - p0).count() / 1000 << "ms" ;
	...
	//	std::chrono::time_point<std::chrono::system_clock> p1 = std::chrono::system_clock::now();
	//	cout << "\tRnet:" << (float)std::chrono::duration_cast<std::chrono::microseconds>(p1 - p0).count() / 1000 << "ms" ;
	}
	if (rnet_res.size()>0){
	//	std::chrono::time_point<std::chrono::system_clock> p0 = std::chrono::system_clock::now();
	...
	//	std::chrono::time_point<std::chrono::system_clock> p1 = std::chrono::system_clock::now();
	//	cout << "\tOnet:" << (float)std::chrono::duration_cast<std::chrono::microseconds>(p1 - p0).count() / 1000 << "ms" ;
	}
	return onet_res;
}
